...
class RedditGraph(TrainedGraph):
    def __init__(self, embeddings=[], graph_precision="FLOAT_32", self_connection=False):
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        dataset = Reddit2(root='data/Reddit')[0]
        embeds = dataset.x if embeddings == [] else embeddings
        super().__init__(dataset=dataset, embeddings=embeds, graph_precision=graph_precision, self_connection=self_connection)
