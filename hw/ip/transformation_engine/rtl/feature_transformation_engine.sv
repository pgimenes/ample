...
for (genvar sys_module = 0; sys_module < SYSTOLIC_MODULE_COUNT; sys_module++) begin
    ...
    always_comb begin
        sys_module_down_in_valid [sys_module*MATRIX_N + (MATRIX_N-1) : sys_module*MATRIX_N] = {MATRIX_N{pulse_systolic_module}} & weight_channel_resp.valid_mask[sys_module*MATRIX_N + (MATRIX_N-1) : sys_module*MATRIX_N];
...
always_comb begin
    ...
    pulse_systolic_module = (fte_state_n == FTE_FSM_MULT_SLOW) && (&aggregation_buffer_fte_out_feature_valid) && weight_channel_resp_valid;

    // Drive systolic module from aggregation buffer (on the left)
    sys_module_forward_valid [0] = slot_pop_shift & {MATRIX_N{pulse_systolic_module}} & busy_aggregation_slots_snapshot;
    sys_module_forward       [0] = aggregation_buffer_fte_out_feature;

    fte_aggregation_buffer_pop = slot_pop_shift & {MATRIX_N{pulse_systolic_module}} & busy_aggregation_slots_snapshot & ~aggregation_buffer_fte_slot_free;
end
