...
module aggregation_mesh #(
    ...
    output logic [AGGREGATION_COLS-1:0]                                                       message_channel_req_valid,
    input  logic [AGGREGATION_COLS-1:0]                                                       message_channel_req_ready,
    output MESSAGE_CHANNEL_REQ_t [AGGREGATION_COLS-1:0]                                       message_channel_req,
    
    // Message Channel: Prefetcher -> AGE (response)
    input  logic [AGGREGATION_COLS-1:0]                                                       message_channel_resp_valid,
    output logic [AGGREGATION_COLS-1:0]                                                       message_channel_resp_ready,
    input  MESSAGE_CHANNEL_RESP_t [AGGREGATION_COLS-1:0]                                      message_channel_resp,
...
logic  [AGGREGATION_COLS:0] [AGGREGATION_ROWS:0] [VC_NUM-1:0]   node_router_on;
logic  [AGGREGATION_COLS:0] [AGGREGATION_ROWS:0]                node_router_valid;
logic  [AGGREGATION_COLS:0] [AGGREGATION_ROWS:0] [VC_NUM-1:0]   node_router_ready;
noc_pkg::flit_t [AGGREGATION_COLS:0] [AGGREGATION_ROWS:0]    node_router_data;
                                                                                                                                       
logic  [AGGREGATION_COLS:0] [AGGREGATION_ROWS:0] [VC_NUM-1:0] router_node_on;
logic  [AGGREGATION_COLS:0] [AGGREGATION_ROWS:0]              router_node_valid;
logic  [AGGREGATION_COLS:0] [AGGREGATION_ROWS:0] [VC_NUM-1:0] router_node_ready;
noc_pkg::flit_t [AGGREGATION_COLS:0] [AGGREGATION_ROWS:0]  router_node_data;

// Aggregation Managers
// ----------------------------------------------------

// NOC signals
logic [AGGREGATION_COLS-1:0]                                                       aggregation_manager_router_on;
logic [AGGREGATION_COLS-1:0]                                                       aggregation_manager_router_valid;
logic [AGGREGATION_COLS-1:0]                                                       aggregation_manager_router_ready;
flit_t [AGGREGATION_COLS-1:0]                                                      aggregation_manager_router_data;
                                                                                                                               
logic [AGGREGATION_COLS-1:0]                                                       router_aggregation_manager_on;
logic [AGGREGATION_COLS-1:0]                                                       router_aggregation_manager_valid;
logic [AGGREGATION_COLS-1:0]                                                       router_aggregation_manager_ready;
flit_t [AGGREGATION_COLS-1:0]                                                      router_aggregation_manager_data;
                                                                                                                               
logic [AGGREGATION_COLS-1:0]                                                       agm_req_valid;
logic [AGGREGATION_COLS-1:0]                                                       agm_req_ready;
AGE_AGM_REQ_t                                                                      agm_req;

// Buffer Manager Allocation
logic [AGGREGATION_COLS-1:0]                                                       buffer_manager_allocation_valid;
logic [AGGREGATION_COLS-1:0]                                                       buffer_manager_allocation_ready;
logic [$clog2(AGGREGATION_ROWS)-1:0]                                               buffer_manager_allocation;

top_pkg::NSB_AGE_REQ_t [AGGREGATION_COLS-1:0]                                      agm_allocation;
logic [AGGREGATION_COLS-1:0] [AGC_COUNT-1:0]                                       agm_allocated_agcs;
logic [AGGREGATION_COLS-1:0] [$clog2(MAX_AGC_PER_NODE)-1:0]                        agm_allocated_agcs_count;
logic [AGGREGATION_COLS-1:0] [MAX_AGC_PER_NODE-1:0] [$clog2(AGGREGATION_COLS)-1:0] agm_coords_buffer_x;
logic [AGGREGATION_COLS-1:0] [MAX_AGC_PER_NODE-1:0] [$clog2(AGGREGATION_ROWS)-1:0] agm_coords_buffer_y;
...
logic [AGGREGATION_COLS-1:0] agm_allocation_oh;
logic [$clog2(AGGREGATION_COLS)-1:0] agm_receiving_buffer_manager_allocation;

logic                        agm_waiting;
logic                        bm_waiting;

logic [AGGREGATION_COLS-1:0]         agm_sending_done_oh;
logic [$clog2(AGGREGATION_COLS)-1:0] agm_sending_done_bin;

// Other
// ----------------------------------------------------

// for visibility in the waveform
logic [VC_NUM-1:0] error_o [AGGREGATION_COLS:0][AGGREGATION_ROWS:0][PORT_NUM-1:0];
...
for (genvar agc_row = 0; agc_row < AGGREGATION_ROWS; agc_row = agc_row + 1) begin : row_gen
    ...
    for (genvar agc_col = 0; agc_col < AGGREGATION_COLS; agc_col = agc_col + 1) begin : col_gen
        ...
        always_comb begin
            aggregation_core_router_on    [agc_row][agc_col] = node_router_on                [agc_col][agc_row][0];
            aggregation_core_router_ready [agc_row][agc_col] = node_router_ready             [agc_col][agc_row][0];
            ...
            router_node_on                [agc_col][agc_row][0] = router_aggregation_core_on    [agc_row][agc_col];
            router_node_ready             [agc_col][agc_row][0] = router_aggregation_core_ready [agc_row][agc_col];
        end
    end : col_gen
end : row_gen
...
for (genvar bm = 0; bm < AGGREGATION_ROWS; bm++) begin : bm_block
    ...
    always_comb begin
        buffer_manager_router_on    [bm]                   = node_router_on              [AGGREGATION_COLS][bm][0];
        buffer_manager_router_ready [bm]                   = node_router_ready           [AGGREGATION_COLS][bm][0];
        node_router_valid           [AGGREGATION_COLS][bm] = buffer_manager_router_valid [bm];
        node_router_data            [AGGREGATION_COLS][bm] = buffer_manager_router_data  [bm];

        router_buffer_manager_valid [bm]                      = router_node_valid           [AGGREGATION_COLS][bm];
        router_buffer_manager_data  [bm]                      = router_node_data            [AGGREGATION_COLS][bm];
...
rr_arbiter #(
    .NUM_REQUESTERS (AGGREGATION_COLS)
) free_agm_arbiter (
    .clk        (core_clk),
    .resetn     (resetn),
...
assign buffer_manager_allocation_valid = agm_allocation_oh & {AGGREGATION_COLS{bm_waiting}};
